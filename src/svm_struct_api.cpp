...
void  read_instance_struct(char *line, PATTERN &x, LABEL &y) {
    ...
    for(int i = 0; i<word->row_num; i++){
        ...
        for(int j = 0; j<word->col_num; j++){
            val.push_back(read_num<float>(line,pos)/255.0);
            total++;
        }
...
void        init_struct_model(SAMPLE sample, STRUCTMODEL *sm, 
    ...
    // NOTE: just fixed #classes and size of box
    sm->num_multilabel = 10;
    sm->bw = 28;
    sm->bh = 28;
    sm->sizePsi = sm->num_multilabel * sm->bw * sm->bh;
    sparm->num_multilabel = sm->num_multilabel;
    sparm->bw = sm->bw;
    sparm->bh = sm->bh;
...
LABEL       classify_struct_example(PATTERN x, STRUCTMODEL *model,
    ...
    double score;
    int *bbx = new int[3];
    for (int i = 0; i < model->num_multilabel; ++i) {
        for(int hi = 0; hi + bh < h; hi += 1){
            for(int wi = 0; wi + bw < w; wi +=1){
                bbx[0] = i;
                bbx[1] = wi;
                bbx[2] = hi;
                DOC doc;
...
LABEL       find_most_violated_constraint_marginrescaling(PATTERN x, LABEL y, 
    ...
    double score = 0;
    int *bbx = new int[3];
    for (int class_id = 0; class_id < model->num_multilabel; ++class_id) {
        for(int hi = 0; hi + bh < h; hi += 1){
            for(int wi = 0; wi + bw < w; wi += 1){
                bbx[0] = class_id;
                bbx[1] = wi;
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  if (y.y[0] != ybar.y[0]) { // wrong label
      return 100;
  }
  else { // right label, consider iou
...
double iou(int *yt, int *ypre, STRUCT_LEARN_PARM *sparm){
    ...
    int lx1 = yt[1]; int rx1 = lx1 + bw;
    int ly1 = yt[2]; int ry1 = ly1 + bh;
    int lx2 = ypre[1]; int rx2 = lx2 + bw;
    int ly2 = ypre[2]; int ry2 = ly2 + bh;
...
void        write_struct_model(char *file, STRUCTMODEL *sm, 
    ...
    fprintf(fl, "%d # number of classes\n", sm->num_multilabel);
...
STRUCTMODEL read_struct_model(char *file, STRUCT_LEARN_PARM *sparm)
{
  /* Reads structural model sm from file file. This function is used
    ...
    free(line);
    sm.svm_model = model;
    sm.num_multilabel = num_classes;
    sm.bw = bw;
    sm.bh = bh;
    sm.w = NULL;
    sparm->num_multilabel = num_classes;
    sparm->bw = bw;
    sparm->bh = bh;
    return sm;
}
